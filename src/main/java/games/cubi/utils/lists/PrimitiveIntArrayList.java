...
public class PrimitiveIntArrayList {
    ...
    public static int get(int@IntArrayListMarker[] array, int index) {
        if (array == null) throw new IndexOutOfBoundsException("Attempted to find value at index: " + index + ", for IntArrayList with size: 0");
        if (index < 0 || index >= array.length) throw new IndexOutOfBoundsException("Attempted to find value at index: " + index + ", for IntArrayList with size: " + size(array));
        return array[index];
    }
